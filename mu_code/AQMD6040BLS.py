...
class AQMD6040BLS(SlaveStation):
    ...
    # 定义初始化函数
    def __init__(self, name, address = 0x01, baud = 115200):
        # 调用父函数初始化
        ...
        # 设置串口参数
        # 需要与设备设置保持一致
        self._set_port(baud, serial.EIGHTBITS,
    ...
    def read_info(self):
        ...
        # 地址从0x0000开始
        # 设备描述信息寄存器共14个
        buffer = SlaveStation.read_regs(self, 0x0000, 14)
        # 检查结果
    ...
    def read_status(self):
        ...
        # 地址从0x0020开始
        # 设备实时状态寄存器共27个
        buffer = SlaveStation.read_regs(self, 0x0020, 27)
        # 检查结果
    ...
    def read_spctl(self):
        ...
        # 地址从0x0050开始
        # 设备实时状态寄存器共4个
        buffer = SlaveStation.read_regs(self, 0x0050, 4)
        # 检查结果
    ...
    def read_mtcfg(self):
        ...
        # 地址从0x0060开始
        # 设备实时状态寄存器共27个
        buffer = SlaveStation.read_regs(self, 0x0060, 27)
        # 检查结果
        if buffer is None or len(buffer) != 54:
            # 打印信息
            if (self.debug) : \
    ...
    def read_syscfg(self):
        ...
        # 地址从0x0080开始
        # 设备实时状态寄存器共30个
        buffer = SlaveStation.read_regs(self, 0x0080, 30)
        # 检查结果
    ...
    def read_poscfg(self):
        ...
        # 地址从0x00A0开始
        # 设备实时状态寄存器共11个
        buffer = SlaveStation.read_regs(self, 0x00A0, 11)
        # 检查结果
    ...
    def read_spcfg(self):
        ...
        # 地址从0x00B0开始
        # 设备实时状态寄存器共4个
        buffer = SlaveStation.read_regs(self, 0x00B0, 4)
        # 检查结果
        if buffer is None or len(buffer) != 8:
            # 打印信息
            if (self.debug) : \
    ...
    def read_pidcfg(self):
        ...
        # 地址从0x00BA开始
        # 设备实时状态寄存器共18个
        buffer = SlaveStation.read_regs(self, 0x00BA, 18)
        # 检查结果
    ...
    def read_lncfg(self):
        ...
        # 地址从0x00E0开始
        # 设备实时状态寄存器共16个
        buffer = SlaveStation.read_regs(self, 0x00E0, 16)
        # 检查结果
    ...
    def read_sfcfg(self):
        ...
        # 地址从0x0100开始
        # 设备实时状态寄存器共10个
        buffer = SlaveStation.read_regs(self, 0x0100, 14)
        # 检查结果
        if buffer is None or len(buffer) != 28:
            # 打印信息
            if (self.debug) : \
    ...
    def read_cancfg(self):
        ...
        # 地址从0x0120开始
        # 设备实时状态寄存器共10个
        buffer = SlaveStation.read_regs(self, 0x0120, 10)
        # 检查结果
    ...
    # 电机学习
    def init_motor(self, force = False):
        # 打印信息
        print("AQMD6040BLS.init_motor : change protection status !")            
        # 设置霍尔错误时间
        self[0x0105].set(100)
        # 写入数据
        if not self.write_reg(0x0105):
            if (self.debug) : \
                print("AQMD6040BLS.init_motor : fail to write R[0105] !")
            return False

        # 打印信息
        print("AQMD6040BLS.init_motor : check status !")            
        # 读取寄存器状态
        if not self.read_reg(0x0075):
            if (self.debug) : \
        ...
        print("\t%s"%self[0x0075])
        # 检查标记位
        if not force and self[0x0075].get() == 1:
            if (self.debug) : \
                print("AQMD6040BLS.init_motor : already initialize !")
            return True
        # 设置数值
        self[0x0075].set(0) #设置为未学习状态
        # 写入寄存器
        if not self.write_reg(0x0075):
            if (self.debug) : \
                print("AQMD6040BLS.init_motor : fail to write R[0075] !")
            return False
        # 读取学习命令
        if not self.read_reg(0x00E1):
            if (self.debug) : \
    ...
    # 设置位置
    def set_pos(self, pos, mode = 0):
        # 打印信息
        print("AQMD6040BLS.set_pos : try to set position !")
        # 设置数值
        self[0x0045].set(mode) #绝对位置
        self[0x0046].set(pos)
        # 打印寄存器数据
        print("\t%s"%self[0x0045])
        print("\t%s"%self[0x0046])
        # 写入寄存器
        if not self.write_reg(0x0045, 2):
            if (self.debug) : \
                print("AQMD6040BLS.set_pos : fail to write R[0045] ~ R[0046] !")
            return False
        if (self.debug) : \
            # 打印信息
            print("AQMD6040BLS.set_pos : position was set !")
        return True

    # 设置通讯波特率
    def set_baud(self, baud):
        # 打印信息
        print("AQMD6040BLS.set_baud : try to set baudrate !")
        # 设置数值
        self[0x0090].set(baud)
        # 打印寄存器数据
        print("\t%s"%self[0x0090])
        # 写入寄存器
        if not self.write_reg(0x0090):
            print("AQMD6040BLS.set_baud : fail to write R[0090] !")
            return False
        # 打印信息
        print("AQMD6040BLS.set_baud : baudrate was set !")
        return True
        
# 定义主函数
def main():  
    # 创建设备
    myDevice = AQMD6040BLS("/dev/ttyUSB0", 0x05, 9600)
    # 修改设备的波特率
    if myDevice.set_baud(115200):
        # 打印信息
        print("AQMD6040BLS.main : 成功设置波特率！")
    else:
        # 打印信息
        print("AQMD6040BLS.main : 未能设置波特率！")

    # 删除设备
    del myDevice
    # 重新创建设备
    myDevice = AQMD6040BLS("/dev/ttyUSB0", 0x05)
    # 逐个读取所有寄存器
    # 速度比较慢
    # myDevice.read_items()
    # 按区快速读取寄存器
